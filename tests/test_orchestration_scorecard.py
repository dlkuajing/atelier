...
- EFL `target=None` 但 FOV+MTF 可用 → ranked（`_rank` 直测——见下方说明，
  真实 `TargetSpec.efl_mm` 恒必填，此路径通过直接单测 `_rank` 覆盖）。
...
关于 `_rank` 直测：`TargetSpec.efl_mm`/`fov_deg`/`fnum` 都是必填非 Optional
字段（`candidate.py`），因此 `score_candidate` 通过真实 `TargetSpec` 永远
不会产出 `field="efl"` 且 `target=None` 的 `TargetDeviation`——spec §9 描述
的"EFL target=None 但 FOV+MTF 可用→ranked"场景是在测试 `RankResult` 覆盖率
门算法本身的通用性，不是在测试 `TargetSpec` 的可选性。因此这类边界场景
直接构造 `TargetDeviation`/`ImageQualityMetrics` 单测 `_rank`（模块私有
函数，代码库既有先例：`generators.py` 直接测试 `case_library._candidate_scenarios`）。
"""
...
def test_imh_row_absent_when_achieved_unavailable():
    """Real payload: `metadata.image_height_mm` is None across the whole
    library today -> no IMH row (not required for coverage, §7-E)."""
    assert _CASE.metadata is not None and _CASE.metadata.image_height_mm is None
    target = _wide_target_spec()
    generated = _generated_candidate()
    row = score_candidate(generated, target)
    assert not any(d.field == "imh" for d in row.target_deviations)
    # absence doesn't block ranking (imh isn't a required metric)
