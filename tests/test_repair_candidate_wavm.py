...
def test_pads_three_slot_file_only_when_write_is_enabled(tmp_path: Path) -> None:
    ...
    assert repair_wavm_file(path) == 21
    with path.open(encoding="ascii", newline="") as handle:
        assert handle.read() == original
    assert repair_wavm_file(path, write=True) == 21
    lines = path.read_text(encoding="ascii").splitlines()
    wavm = [line for line in lines if line.startswith("WAVM ")]
    assert len(wavm) == 24
    assert wavm[:3] == original.splitlines()[1:4]
...
def test_already_24_slot_file_is_idempotent(tmp_path: Path) -> None:
    ...
    assert repair_wavm_file(path, write=True) == 0
    assert path.read_text(encoding="ascii") == content
