...
- `origin/main` = `d7b0b897`（#101 合入后再更新）
- 本轮已合入：**#93**（多波长导入根治 + 四指标种子值 fail-closed）、**#94**（可追迹率普查）、
  **#95**（CODE V 路径归一化，异底座审查捕获的 #93 回归）、**#96**（分母更正 + 本文件）、
  **#97**（本文件刷新）、**#98**（处方保真度审计 + 语料闸）、**#99**（P2 样本量普查）、**#100**（`mtf_drop` 收口）、**#101**（追迹失败根因调查）
- 未做的技术债见 §5 第 4–6 项
...
1. **P2 异源打平率首次实测** —— 北极星主指标**至今零数据**，已升为第 1 位。
   前置数已备齐（PR #99）：对照组 = **192 颗**可用 seed（严格可追迹 ∧ 保真度干净），
   可成立 **167 个异源 trial**。
   ⚠️ **167 不是有效样本量**：只用到 **31 颗 seed**，top-5 占 59.3%。报数必须两个数同报。
   **卡点不是缺优化能力**——`run_codev_target` / `_autovig` / `_standard` / `_fno_ladder`
   四个入口就在 main 上（PR #45 已合，2026-07-28 实测确认；此前记忆里「未合 main」是错的）。
   真正的卡点是：① **配对协议未裁定**（每对照取 1 颗还是 k 颗？按 spec 分层配额？
   seed 复用集中说明「每对照取最近 1 颗」会让指标被少数 seed 主导）
   ② 真机跑批时间 ③ **对拍器不存在**（候选 vs 对照在 RMS/MTF/畸变三项的比较层要新建；
   注意它目前**没有消费者**，属 §2 的「造件 ahead-of-consumer」上交项）。
   **做出第一版数字比数字好看重要**；门槛数值**禁止先验拍板**。
