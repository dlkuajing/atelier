...
1. **343 颗全失败 + 256 颗半残的根因** —— 直接决定 456 能回升多少。**七条假设已用数据排除**
   （渐晕 / `RAIM` / 视场角 / 片数 / `CRADJ`，**别重走，那是几十次真机 run**；
   PR #98 又排除**后段口径缩水**与**非球面丢失**两条）。
   最锋利的事实：① 失败**集中在边缘视场**（轴上 `err=0` 但点列径恰好 `0.0`）；
   ② **所有 setup 侧杠杆均无效**；③ **两个独立生成批次呈现相似失败结构**；
   ④ **转换期就已注定** —— 收据里的 `trace_audit.finite_final_rays` 强预测 CODE V 结果
   （5/5 → 76.3% full，<5 → 11.0% full，n=752 次尝试），而离线 Optiland 的
   `aperture` 为 `None`（`DIAM` 不裁切）+ 暂存期半口径 100mm，故阵亡是**几何/物理性的**，
   不是口径造成的；阵亡位置全在系统中段（17–19 面的第 8–16 面），**轴上主光线 0/60 阵亡**。
   → 嫌疑仍在 `scripts/patent_to_zmx.py`，但已收敛到**实光线行为**（高阶项 / 光阑与光瞳 setup），
   **不在近轴数据**（EFL 复算与声明值在 551/613 上吻合到 2% 内）。
   **下一步验证**：拿失败 seed 处方与专利原表**逐面对拍**（仍未做）
2. **P2 异源打平率首次实测** —— 北极星主指标当前**零数据**。对照组用可用 seed
   （**今天可路由且保真度干净的是 192**，不是 218；若走 staging 需先解决其 ledger 的
   fail-closed 提升条件），同族判定用专利家族。
   **对照组必须先跑 `scripts/corpus_fidelity_audit.py` 摘掉 135 颗缺陷 seed**，
   否则第一个数就是虚高的。**做出第一版数字比数字好看重要**
