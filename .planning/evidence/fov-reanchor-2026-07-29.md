...
混着半视场与全视场两种口径；本铲把 **254 颗**重写为 `2θ`（θ = 该 case 自己 ZMX 的外场角）。
**配对质量实测改善：seed/对照视场角落在 0.8–1.25× 的比例 60.0% → 89.1%，最坏 2.046× → 1.369×。**
...
| 值实际改变 | **254** |
| 已经是对的 | 171 |
| 文件 diff | 255 文件 / +508 −508 —— **每文件恰好一行** |
| 幂等 | 重跑 `--check` 报 `would change 0` |

per-case JSON 必须一起动：`load_case_library()` 读的是 per-case 文件、**不读 index.json**，
且在 load 时用 `fov_deg` 重算 scenario。只改 index 会让路由与报表分叉。
...
|---|---|---|
| 可用 trial | 30 | **46** |
| seed/对照 视场角比 中位 | 1.067 | 1.091 |
| 同上 p25 / p75 | 0.491 / 1.106 | **1.059 / 1.175** |
| min / max | 0.473 / **2.046** | 0.747 / **1.369** |
| 落在 0.8–1.25× | 18/30 = 60.0% | **41/46 = 89.1%** |
