...
⚠️ **要分两层看**：这道闸是 RMS / MTF / floor-gap **三条 OR**，本 PR 只改了 RMS 那条。
已经因 MTF 或 floor-gap 判违规的行**不可能再翻**，所以**支路数会高估影响 45%**。
**该引用的是闸级数字**：

| | healthy→violation | violation→healthy |
|---|---|---|
| 只换仪器 · **RMS 支路** | 16 | 9 |
| 只换仪器 · **闸级（本 PR 的真实影响）** | **11** | **3** |
| 再把闸降到 10.2312（P2 已在用的语料中位） | 111 | 0 |

闸级违规总数 **174 → 182**（共 442 行）；442 行里 **151 行**本来就因 MTF/floor-gap 判违规，
其中 **5** 条 RMS 支路的翻转被它们遮住。

⚠️ **本页第一版只报了支路数（16/9）而把它称作「代价」——那描述的不是这道闸。**
更正过程本身也踩了一次坑：重算脚本里写了 `2.0 * old_rms if old_rms else None`，
真值判断把 **72 行存 `0.0` 的 RMS** 当成「没有读数」，一度算出 88/9。
`0.0` 在这里读起来像一颗完美镜头——正是本项目记录在案的「退化值等于理想读数」那一格。
