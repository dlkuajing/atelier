...
Atelier 是一个可独立部署的产品级光学设计 Agent 软件：客户在浏览器界面里用自然语言输入光学需求，系统实时给出设计（光路图 / MTF / 专家级评估报告），并展示由 Code V 深度优化产出的专业级设计成果。目标用户是镜头/模组厂的工程团队与决策者，用于现场客户演示。它从 lumira 官网后端剥离而来，现在是独立产品线，与官网可不互通。
