...
## PR CI fix-forward — 2026-07-13

- PR #81 的首轮 CI run `29225939487` 在 head `ca108a567fd380f15c33129d892d4a80411ef7c9` 上失败：`2700 passed / 17 skipped / 2 failed`。失败节点仅为 `test_attested_v3_bundle_snapshots_full_package_and_same_candidate_bytes` 与 `test_candidate_for_seed_production_stagec_uses_accepted_bytes_for_attested_payload`；cache 400 与 Node 20 deprecation 只是 workflow 告警，不是测试根因。
- 两项均为 Ubuntu fixture 泄漏 Windows 默认路径，不是 Stage C lock/receipt 生产校验回归。export fixture 一端保留 lexical `D:/CODEV115/codev.exe`、另一端用宿主 `Path.resolve()`，在 Linux 分裂为 Windows key 与 POSIX key，恢复器于 executable binding 正确 fail-closed；generator fixture 虽替换 Stage B authority executable，却漏替换模块已导入的 lazy `DEFAULT_CODEV_EXECUTABLE`，因而在进入 fake authority 前 strict-resolve `D:` 失败。
- 最小修复只改测试：export fixture 写入与 pinned digest 同字节的 `tmp_path/codev.exe`，并同时绑定 attested 与 Stage B authority；generator fixture 显式注入同一 `fake_codev`，且断言传入 `StageBAuthorityConfig.executable` 精确等于该 fixture。生产路径、可信 pin、锁与 restore fail-closed 逻辑均未放宽。
- fix-forward 本地回归：两个原失败节点 `2 passed`；export+generator 两文件 non-real 全集 `79 passed / 4 deselected`；stagec-attested+export+generator 三文件 non-real 全集 `152 passed / 19 deselected`；两文件 `ruff check`、`ruff format --check` 与 `git diff --check` 均 PASS。三路互相独立的只读根因审计一致判定 fixture-only，最终 diff 未发现 P0–P3。Ubuntu 真值仍须由新的 PR CI success 决定，禁止复用或盲重跑失败 run。
