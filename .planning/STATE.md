...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest exact Genius profile expands one nine-lens document failure into 11 optical/asphere-bound embodiment rejections from a pinned 65-page/48-sheet official PDF and exact FIG.8–50 map. Canonical attempts 3/4 agree. Generic bucket is 235, result set `0da2ea63...6457`, summary `89017c49...897f`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (235), using the same strict before/after census, source-proven
