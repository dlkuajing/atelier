...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. After three Ability official-PDF exact-image OCR profiles: 422 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items remain non-terminal. `US-11231565-B2:e1` is source-proven `metadata_unpublished`; `US-11175479-B2` is split into three source-bound OCR failures. The largest parser bucket remains `generic_summary_metadata_missing=258`. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Layouts 1--17, four source-terminal publications, five exact-raster Ability PDF profiles, and the source-bound B2→A1 inverse prior-publication path are replayed append-only. `US-11231565-B2:e1` plus `US-10690884-B2:e1/e2` are terminal `metadata_unpublished`; `US-11175479-B2` retains three parser items while `US-20200201001-A1` and `US-11768354-B2` retain two each below unchanged OCR gates. TIFF container-padding nondeterminism is removed by canonical decoded-raster hashing. Latest attempts 2/3 have identical evidence/results and launch no worker. Generic bucket is 255, result set `ddf8aa92...c28c`, summary `1645bf2b...043`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`.planning/quick/260715-patent-generic-summary-metadata-parser/260715-patent-generic-summary-metadata-parser-PLAN.md`,
