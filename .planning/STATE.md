...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest exact Family ID 79321029 classifier proves two multi-camera imaging-lens-system disclosures contain no surface prescription; attempts 2/3 agree and no conversion worker is created. Generic bucket is 227, result set `ad918605...525df`, summary `bb30a536...f6f9a`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (227), using the same strict before/after census, source-proven
