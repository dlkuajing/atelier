...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest exact Family ID 98700212 classifier proves two barcode-reader disclosures contain no prescription; attempts 2/3 agree and no conversion worker is created. Generic bucket is 229, result set `cc7c29c7...b0fd`, summary `a77a2c20...1b8e`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (229), using the same strict before/after census, source-proven
