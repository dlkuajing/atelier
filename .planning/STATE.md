...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 414 parser-review roots, 139 mixed roots, 46 terminal roots, 518 staging items, and 28 conversion-retry items. The latest exact Genius profile expands `US-20250020895-A1` into 14 independently retained eight-lens embodiment rejections from a 64-page, 46-sheet pixel-verified PDF pair. Generic-summary remains the largest parser bucket at 232 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest Genius profile binds 14 eight-lens optical/asphere pairs plus FIG.62A/B and retains 30 key pages from official/Google PDFs whose 64 decoded rasters are all equal. Attempts 2/3 agree and all 14 items remain parser review at unchanged OCR gates. Generic bucket is 232, result set `3099e9cb...e9b1`, summary `6334ad3c...9105`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (232), using the same strict before/after census, source-proven
