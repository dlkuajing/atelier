...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 418 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items. The first Genius image-table root now retains all 11 embodiments separately at unchanged OCR gates; generic-summary remains the largest parser bucket at 246 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, and B2→A1 prior-publication recovery are replayed append-only. Latest Genius evidence expands `US-20170097490-A1` from one document rejection to 11 optical/asphere/Fno-bound embodiment rejections without numeric repair or worker launch. Canonical attempts 2/3 agree. Generic bucket is 246, result set `427df235...bd7a`, summary `85c42e3b...56aa`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (246), using the same strict before/after census, source-proven
