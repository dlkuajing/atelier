...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 418 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items. Three related Genius six-lens publications now retain five embodiments each; two use decoded-raster-identical overlays and the newest uses explicitly labelled official-PDF-only coordinate OCR because no public Google citation PDF exists. Generic-summary remains the largest parser bucket at 243 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and an explicitly distinct official-PDF-only OCR path are replayed append-only. Latest Genius six-lens family evidence expands three document failures into 15 optical/asphere-bound embodiment rejections without numeric repair or worker launch. Canonical attempts 3/4 agree per root. Generic bucket is 243, result set `24859a64...8b9b`, summary `894fd534...aded`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (243), using the same strict before/after census, source-proven
