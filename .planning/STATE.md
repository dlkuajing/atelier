...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 418 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items. The latest Genius four-lens document now retains nine embodiments from an independently pinned 47-page official PDF with a proven 33-sheet layout and no mirror claim. Generic-summary remains the largest parser bucket at 238 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest Genius four-lens profile expands one document failure into nine optical/asphere-bound embodiment rejections from a pinned 47-page/33-sheet PDF and exact FIG.8–45 map. Canonical attempts 3/4 agree. Generic bucket is 238, result set `ecf14c32...281b`, summary `c28bbe65...520d`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (238), using the same strict before/after census, source-proven
