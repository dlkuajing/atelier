...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 418 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items. Two further Genius six-lens documents now retain nine embodiments each from independently pinned 48/50-page official PDFs with proven 33/34-sheet layouts and no mirror claim. Generic-summary remains the largest parser bucket at 239 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. Latest Genius variants expand two document failures into 18 optical/asphere-bound embodiment rejections across independently pinned 48/50-page PDFs with exact 33/34-sheet and 3/4-comparison-page maps. Canonical attempts 3/4 agree per root. Generic bucket is 239, result set `7bdd16ec...0f8b`, summary `4426f2ac...9121`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (239), using the same strict before/after census, source-proven
