...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 414 parser-review roots, 139 mixed roots, 46 terminal roots, 518 staging items, and 28 conversion-retry items. The latest exact-source shovel classifies the A1/B2 records of NEWMAX Family ID 79907355 independently as non-optical-zone stray-light architecture with no published prescription. Generic-summary remains the largest parser bucket at 233 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and explicit official-PDF-only OCR paths are replayed append-only. The latest shovel pins both official records carrying NEWMAX Family ID 79907355, proves zero PPUBS prescription tables, and classifies each root as `confirmed_no_prescription.non_optical_zone_stray_light_architecture_only`; canonical attempts 2/3 agree. Generic bucket is 233, result set `dd8c33bc...2369`, summary `a32a8ddc...24a5`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (233), using the same strict before/after census, source-proven
