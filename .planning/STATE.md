...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Layouts 1--17, four source-terminal publications, four exact-raster Ability PDF profiles, and the source-bound B2→A1 inverse prior-publication path are replayed append-only. `US-11231565-B2:e1` is terminal `metadata_unpublished`; `US-11175479-B2` retains three parser items while `US-20200201001-A1` and `US-11768354-B2` retain two each below unchanged OCR gates. TIFF container-padding nondeterminism is removed by canonical decoded-raster hashing. Latest attempts 2/3 have identical evidence/results and launch no worker. Generic bucket is 256, result set `bde5ef26...a84a`, summary `8147f483...e86a`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
