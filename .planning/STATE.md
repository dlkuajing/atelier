...
| Patent saturation | ACTIVE / INCOMPLETE；619-root frozen local replay remains strict with missing=0 and corrupt=0. After source-terminal classification and generic-summary layout 16: 424 parser-review roots, 138 mixed roots, 517 staging items, and 28 patent-budget retry items remain non-terminal. Terminal evidence now includes 1 confirmed-no-prescription document and 10 metadata-unpublished embodiments. The largest parser bucket remains `generic_summary_metadata_missing=261`; this is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Layouts 1--16 plus three source-terminal publications are replayed append-only. Layout 16 parses 9/10 Samsung even-order embodiments; the official damaged S11 header remains rejected, while two full 1200-second replays preserve all request/state/ZMX hashes. Latest items are 3 staging, 5 trace-failed, 1 trace-timeout, and 1 parser reject. Generic bucket is 261, result set `f527823e...3bc0`, and audit is 619/619 corrupt=0. Select the next measured failure bucket without promoting staging. |
...
`generic_summary_metadata_missing` (261), using the same strict before/after census, source-proven
