...
| Patent saturation | ACTIVE / INCOMPLETE. Frozen 619-root local replay remains strict with missing=0 and corrupt=0. Current evidence has 418 parser-review roots, 139 mixed roots, 518 staging items, and 28 conversion-retry items. The latest Genius six-lens family retains nine embodiments independently for both its 50-page grant and 51-page continuation publication, using official-PDF-only coordinate OCR with no mirror claim. Generic-summary remains the largest parser bucket at 241 roots. This is not source/global saturation. |
...
| `260715-patent-generic-summary-metadata-parser` | active-next-layout | Strict before census 294 items/294 roots. Source-proven HTML layouts, source terminals, exact-raster PDF profiles, B2→A1 prior-publication recovery, and an explicitly distinct official-PDF-only OCR path are replayed append-only. Latest Genius nine-embodiment family expands two document failures into 18 optical/asphere-bound embodiment rejections across independently pinned 50/51-page official PDFs. Canonical attempts 3/4 agree per root. Generic bucket is 241, result set `b65b39ad...1d17`, summary `ade80783...8977`, and audit is 619/619 corrupt=0. Continue the largest measured parser bucket without promoting staging. |
...
`generic_summary_metadata_missing` (241), using the same strict before/after census, source-proven
