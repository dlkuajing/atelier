...
# Routing-winner golden (E2-01 hybrid contract): the volatile per-brief expected
# seed + quality evidence live in a golden file regenerated by scripts/e2_golden.py
# after each ingest batch; the behaviour contracts below stay hardcoded. See
...
def _floor_aware_performance_seed_selected() -> Check:
    def check(sample: OpticalSampleData) -> tuple[bool, str]:
        ...
        # Routing winner + quality evidence come from the golden (regenerated per
        # ingest batch); the behaviour contract (met F/#, blocked on the aperture-
        # limited high-frequency floor) stays hardcoded.
        golden = _EVAL_GOLDEN["performance_full_field_seed_blocks_low_mtf"]
        ok = (
            sample.metadata.case_id == golden["selected_case_id"]
            and assessment.recommended_candidate_id == "seed-baseline"
            and quality is not None
            and quality.label == "MTF/RMS floor evidence"
            and f"floor gap {golden['quality_floor_gap']}" in quality.actual
            and "1.0 field" in quality.actual
            and f"min250 {golden['quality_min250']}" in quality.actual
...
def _balanced_floor_aware_seed_selected() -> Check:
    def check(sample: OpticalSampleData) -> tuple[bool, str]:
        ...
        gate_checks = {item.check_id: item for item in gate.checks} if gate else {}
        # Routing winner + quality evidence come from the golden (regenerated per
        # ingest batch); the behaviour contract (blocked on the aperture-limited
        # high-frequency floor) stays hardcoded.
        golden = _EVAL_GOLDEN["balanced_main_default"]
        ok = (
            sample.metadata.case_id == golden["selected_case_id"]
            and assessment.recommended_candidate_id == "seed-baseline"
            and quality is not None
...
EVAL_CASES: tuple[EvalCase, ...] = (
    ...
    EvalCase(
        ...
        checks=(
            ...
            _case_contains(
                _EVAL_GOLDEN["relaxed_full_field_fallback_blocks_low_mtf"]["selected_case_id"]
            ),
...
def _cached_wavefront_summary(source_zmx: str, nominal_fov_deg: float) -> dict[str, Any]:
    source_path = ZMX_AMMO_DIR / source_zmx
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        optic = load_normalized_zmx(source_path)
        regularize_fields_to_angle(optic, nominal_fov_deg)
        metrics = compute_wavefront_metrics(
            optic,
            num_rays=6,
            num_zernike_terms=0,
        )

    max_rms = metrics.max_rms_wavefront_error_waves
    ...
    return {
        ...
        "min_strehl_ratio": min_strehl,
        "strehl_model": metrics.strehl_model,
    }


def _wavefront_packet(sample: OpticalSampleData) -> dict[str, Any] | None:
    metadata = sample.metadata
    if metadata is None:
        return None
    try:
        return _cached_wavefront_summary(metadata.source_zmx, metadata.fov_deg)
    except Exception as exc:  # noqa: BLE001 - optional eval evidence should not mask case result
        return {
            "status": "unavailable",
            "reason": str(exc),
        }
...
def _row_packet(
    ...
    return {
        ...
        "wavefront_metrics": _wavefront_packet(sample),
